...
//  protected
  ...
  equation   
    ...
    //pressure drop
    inlet.p - outlet.p = f * length_cell * rho *  (u ^ 2) / d_h;
  
    //inlet.h_outflow = inStream(outlet.h_outflow);
    
    (inlet.h_outflow - outlet.h_outflow) * inlet.m_flow = Q; 
...
equation  
  ...
  for i in 1 : N_seg loop
    ...
    k_wall[i] = MyUtil.thermal_conductivity(tableID = table_th_inconel_750, name = name_material, temperature = (cell_cold[i].T + cell_hot[i].T) / 2);
