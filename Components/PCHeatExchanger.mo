...
algorithm  
  ...
  // mass flux
  G_c := outlet_cool.m_flow / N_channel / A_c;
  ...
  // specific enthalpy
  state_cell[1].h_mass_h := CP.PropsSI("H", "P", state_cell[1].p_h, "T", state_cell[1].T_h, state_cell[1].medium_name_h);
  state_cell[1].h_mass_c := CP.PropsSI("H", "P", state_cell[1].p_c, "T", state_cell[1].T_c, state_cell[1].medium_name_c);  
  ...
  for i in 1 : N_seg loop // index out of range??
    ...
    state_cell[i].u_c := outlet_cool.m_flow / A_stack / state_cell[i].rho_c; 
    ...
    debugInfo({"i+1","h_mass_h[i]","m_dot_inlet_hot","q[i]"},{i+1,state_cell[i].h_mass_h,state_cell[i].mdot_h, state_cell[i].q})); 
 
    state_cell[i + 1].h_mass_c := (state_cell[i].h_mass_c * state_cell[1].mdot_c - state_cell[i].q) / state_cell[1].mdot_c;
