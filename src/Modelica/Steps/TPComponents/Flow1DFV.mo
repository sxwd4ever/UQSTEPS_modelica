...
model Flow1DFV "1-dimensional fluid flow model for gas (finite volumes) - my version which ignore the composition change"
  ...
  Medium.ThermodynamicState gas[N] "Thermodynamic state of the gas nodal properties";
...
equation
  ...
  for j in 1:N - 1 loop
    if not QuasiStatic then
      ...
      // + vector(drbdX1[j, :])*vector(der(gas[j].X)) + vector(drbdX2[j, :])*vector(der(gas[j + 1].X))) "Mass balance";
  ...
  */
  for j in 1:N loop
    ...
    gas[j] = Medium.setState_pT(p, T[j]);
    h[j] = Medium.specificEnthalpy(gas[j]);
    /*
    gas[j].p = p;
    gas[j].T = T[j];
    gas[j].h = h[j];
  ...
  */
  if nXi > 0 then
    infl.Xi_outflow = zeros(nXi) * 1.0;
    outfl.Xi_outflow = zeros(nXi) * 1.0;
  end if;
...
initial equation
  if initOpt == ThermoPower.Choices.Init.Options.noInit or QuasiStatic then
    // do nothing
