...
model TP_RCBCycleGUI "Replica of Recompressoion Brayton Cycle transient simulation based on ThermoPower. For layout design purpose"
  ...
  /*
        ...
        */
  ThermoPower.Gas.SourcePressure source(redeclare package Medium = medium_main, T = st_source.T, p0 = st_source.p, use_in_T = false, use_in_p0 = true, gas(p(start = st_source.p, nominal = st_source.p), T(start = st_source.T, nominal = st_source.T))) annotation(
  ...
  /*
          ...
          annotation(
            Placement(transformation(origin = {0, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 270)));
        */
  ThermoPower.Gas.SinkMassFlow sink(redeclare package Medium = medium_main, T = st_sink.T, p0 = st_sink.p, use_in_T = false, use_in_w0 = false, w0 = st_sink.mdot) annotation(
  ...
  Steps.TPComponents.PCHE HTR(redeclare package FluidMedium = medium_main, redeclare package FlueGasMedium = medium_main, redeclare replaceable model HeatTransfer_F = Steps.TPComponents.GnielinskiHeatTransferFV, redeclare replaceable model HeatTransfer_G = Steps.TPComponents.GnielinskiHeatTransferFV, gasFlow(heatTransfer(pitch = cfg_HTR.cfg_hot.l_pitch, phi = cfg_HTR.cfg_hot.a_phi, Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 2000, gamma_max = 5000)), fluidFlow(heatTransfer(pitch = cfg_HTR.cfg_cold.l_pitch, phi = cfg_HTR.cfg_cold.a_phi, Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 2000, gamma_max = 5000)), cfg = cfg_HTR, N_G = N_seg_HTR, N_F = N_seg_HTR, SSInit = true, gasQuasiStatic = false, fluidQuasiStatic = false, table_k_metalwall = cfg.table_k_HTR_wall) annotation(
  ...
  ThermoPower.Water.SinkPressure sink_cooler(redeclare package Medium = medium_cooler, p0 = cfg_cooler.cfg_cold.st_out.p, T = cfg_cooler.cfg_cold.st_out.T, use_T = false) annotation(
  ...
  ThermoPower.Gas.Compressor compressor(redeclare package Medium = medium_main, pstart_in = cfg_comp.st_in.p, pstart_out = cfg_comp.st_out.p, Tstart_in = cfg_comp.st_in.T, Tstart_out = cfg_comp.st_out.T, tablePhic = tablePhic_comp_mc, tableEta = tableEta_comp_mc, tablePR = tablePR_comp_mc, Table = ThermoPower.Choices.TurboMachinery.TableTypes.matrix, Ndesign = cfg_comp.N, Tdes_in = cfg_comp.st_in.T, gas_iso(p(nominal = cfg_comp.st_in.p), T(nominal = cfg_comp.st_in.T), h(start = cfg_comp.st_out.h))) annotation(
  ...
  constant Integer SEC_PER_HOUR = integer(60 * 60 / 12);
  // 5 min = 1 hour
  ...
  Modelica.Blocks.Sources.Ramp ramp_mdot(final duration = 0.15 * SEC_PER_HOUR, final startTime = 1 * SEC_PER_HOUR, final height = -st_sink.mdot * 0.15, final offset = st_sink.mdot) annotation(
    Placement(visible = true, transformation(origin = {-119, 21}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Steps.TPComponents.RampSeq_TwoStages ramp_p(final time_start = 2 * SEC_PER_HOUR, final interval = 0.5 * SEC_PER_HOUR, final duration_1 = 0.15 * SEC_PER_HOUR, final duration_2 = 0.15 * SEC_PER_HOUR, final height_1 = -1e6, final height_2 = -1e6, final offset = st_source.p) annotation(
...
equation
  ...
  connect(turbine.shaft_a, const_speed_turb.flange) annotation(
    Line);
  ...
  connect(recompressor.shaft_a, const_speed_rc.flange) annotation(
    Line);
  ...
  connect(compressor.shaft_a, const_speed_mc.flange) annotation(
    Line);
...
// __OpenModelica_simulationFlags(lv = "LOG_DEBUG,LOG_NLS,LOG_NLS_V,LOG_STATS,LOG_INIT,LOG_STDOUT, -w", outputFormat = "mat", s = "dassl", nls = "homotopy"));
  ...
  connect(ramp_p.y, source.in_p0) annotation(
    Line(points = {{-113, -32}, {-107.5, -32}, {-107.5, -36}, {-108, -36}}, color = {0, 0, 127}));
  connect(const_speed_rc.flange, recompressor.shaft_b) annotation(
    Line(points = {{86, 7}, {74, 7}}));
  connect(const_speed_mc.flange, compressor.shaft_b) annotation(
    Line(points = {{128, 7}, {122.5, 7}, {122.5, 6}, {117, 6}}));
  connect(const_speed_turb.flange, turbine.shaft_b) annotation(
