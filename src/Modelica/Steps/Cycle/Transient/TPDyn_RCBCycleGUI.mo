...
model TPDyn_RCBCycleGUI "Replica of Recompressoion Brayton Cycle transient simulation based on ThermoPower. For layout design purpose"
  ...
  parameter Model.RCBCycleConfig cfg(redeclare package medium_heater = medium_heater, redeclare package medium_main = medium_main, redeclare package medium_cooler = medium_cooler, r_i_heater = 1e-3, r_t_heater = 2e-3, r_o_heater = 3e-3, N_ch_heater = 10000, L_heater = 1, N_ch_HTR = 30000, L_HTR = 2.5, r_i_HTR = 1.5e-3, r_o_HTR = 1.5e-3, N_ch_LTR = 30000, L_LTR = 2.5, r_i_LTR = 1.5e-3, r_o_LTR = 1.5e-3, Ns_turb = 30000, Ns_comp = 30000, Ns_recomp = 30000, N_ch_cooler = 50000, r_i_cooler = 0.5e-3, r_t_cooler = 0.7e-3, r_o_cooler = 1e-3, p_comp_in = 109.59e5, p_comp_out = 20e6, p_heater = 20e6, T_HTR_hot_in = from_degC(556.322), T_HTR_cold_out = from_degC(521.234), T_HTR_hot_out = from_degC(330.103), T_HTR_cold_in = from_degC(303.425), T_LTR_cold_in = from_degC(119.011), T_LTR_hot_out = from_degC(164.419), T_heater_hot_in = from_degC(800), T_heater_hot_out = from_degC(523.547), T_heater_cold_out = from_degC(608.148), T_cooler_cold_out = from_degC(112.138), T_cooler_hot_out = from_degC(59.279), mdot_main = 128.774, mdot_comp = 88.0661, mdot_heater = 40, mdot_cooler = 40.7188);
  // agree with the final parameter Dhyd = 1 in HE, should be checked to see if it is capable of containing all fluid-metal tubes
  ...
  //.cfg_hot.st_out;
  parameter Model.ThermoState st_sink_temp = cfg_cooler.cfg_hot.st_out;
  //.cfg_hot.st_out;
  ...
  ThermoPower.Water.SourceMassFlow source_heater_hot(redeclare package Medium = medium_heater, w0 = cfg_heater.cfg_hot.st_in.mdot, p0 = cfg_heater.cfg_hot.st_in.p, h = cfg_heater.cfg_hot.st_in.h, T = cfg_heater.cfg_hot.st_in.T) annotation(
    Placement(visible = true, transformation(origin = {-74, 4}, extent = {{-4, -4}, {4, 4}}, rotation = 90)));
  ThermoPower.Water.SinkPressure sink_heater_hot(redeclare package Medium = medium_heater, p0 = cfg_heater.cfg_hot.st_out.p, T = cfg_heater.cfg_hot.st_out.T, use_T = false) annotation(
    Placement(visible = true, transformation(origin = {-74, 62}, extent = {{-4, -4}, {4, 4}}, rotation = 90)));
  Steps.TPComponents.HE heater(redeclare package FluidMedium = medium_heater, redeclare package FlueGasMedium = medium_main, redeclare replaceable model HeatTransfer_F = Steps.TPComponents.NgoHeatTransferFV, redeclare replaceable model HeatTransfer_G = Steps.TPComponents.NgoHeatTransferFV, gasFlow(heatTransfer(Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 1000, gamma_max = 5000), Tstartin = cfg_heater.cfg_gas.st_in.T, Tstartout = cfg_heater.cfg_gas.st_out.T, Nt = cfg_heater.cfg_gas.N_ch, Dhyd = cfg_heater.cfg_gas.geo_area.d_hyd), fluidFlow(heatTransfer(Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 2000, gamma_max = 1.5e6), fixedMassFlowSimplified = true, hstartin = cfg_heater.cfg_fluid.st_in.h, hstartout = cfg_heater.cfg_fluid.st_out.h), redeclare model HeatExchangerTopology = ThermoPower.Thermal.HeatExchangerTopologies.CounterCurrentFlow, cfg = cfg_heater, N_G = N_seg_heater, N_F = N_seg_heater, SSInit = true, gasQuasiStatic = false, FluidPhaseStart = ThermoPower.Choices.FluidPhase.FluidPhases.Liquid) annotation(
  ...
  ThermoPower.Gas.SourceMassFlow source(redeclare package Medium = medium_main, T = st_source.T, p0 = st_source.p, w0 = st_source.mdot, use_in_T = false, use_in_w0 = true, gas(p(start = st_source.p, nominal = st_source.p), T(start = st_source.T, nominal = st_source.T))) annotation(
  ...
  */
  ...
  ThermoPower.Gas.SinkPressure sink(redeclare package Medium = medium_main, p0 = st_sink.p, T = st_sink.T, use_in_p0 = true, gas(p(start = st_sink.p, nominal = st_sink.p), T(start = st_sink.T, nominal = st_sink.T))) annotation(
  ...
  */
  ...
  Steps.TPComponents.PCHE LTR(redeclare package FluidMedium = medium_main, redeclare package FlueGasMedium = medium_main, redeclare replaceable model HeatTransfer_F = Steps.TPComponents.NgoHeatTransferFV, redeclare replaceable model HeatTransfer_G = Steps.TPComponents.NgoHeatTransferFV, gasFlow(heatTransfer(pitch = cfg_LTR.cfg_hot.l_pitch, phi = cfg_LTR.cfg_hot.a_phi, Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 2000, gamma_max = 5000)), fluidFlow(heatTransfer(pitch = cfg_LTR.cfg_cold.l_pitch, phi = cfg_LTR.cfg_cold.a_phi, Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 2000, gamma_max = 5000)), cfg = cfg_LTR, N_G = N_seg_LTR, N_F = N_seg_LTR, SSInit = true, gasQuasiStatic = false, fluidQuasiStatic = false) annotation(
  ...
  Steps.TPComponents.PCHE HTR(redeclare package FluidMedium = medium_main, redeclare package FlueGasMedium = medium_main, redeclare replaceable model HeatTransfer_F = Steps.TPComponents.GnielinskiHeatTransferFV, redeclare replaceable model HeatTransfer_G = Steps.TPComponents.GnielinskiHeatTransferFV, gasFlow(heatTransfer(pitch = cfg_HTR.cfg_gas.l_pitch, phi = cfg_HTR.cfg_gas.a_phi, Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 2000, gamma_max = 5000)), fluidFlow(heatTransfer(pitch = cfg_HTR.cfg_fluid.l_pitch, phi = cfg_HTR.cfg_fluid.a_phi, Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 2000, gamma_max = 5000)), cfg = cfg_HTR, N_G = N_seg_HTR, N_F = N_seg_HTR, SSInit = true, gasQuasiStatic = false, fluidQuasiStatic = false) annotation(
  ...
  Steps.TPComponents.GasStateReader r_HTR_cout(redeclare package Medium = medium_main, gas(p(start = cfg_HTR.cfg_cold.st_out.p, nominal = cfg_HTR.cfg_cold.st_out.p), h(start = cfg_HTR.cfg_cold.st_out.h, nominal = cfg_HTR.cfg_cold.st_out.h))) annotation(
    Placement(visible = true, transformation(origin = {-22, 16}, extent = {{-4, -4}, {4, 4}}, rotation = -90)));
  ThermoPower.Gas.Turbine turbine(redeclare package Medium = medium_main, fileName = Modelica.Utilities.Files.loadResource("modelica://Steps/Resources/Data/turbine_map.txt"), tablePhic = fill(0.0, 14, 12), tableEta = fill(0.0, 14, 12), pstart_in = cfg_turb.st_in.p, pstart_out = cfg_turb.st_out.p, Tstart_in = cfg_turb.st_in.T, Tstart_out = cfg_turb.st_out.T, Ndesign = cfg_turb.N, Tdes_in = cfg_turb.st_in.T, Table = ThermoPower.Choices.TurboMachinery.TableTypes.file, gas_in(p(nominal = turbine.pstart_in), T(nominal = turbine.Tstart_in), h(start = cfg_turb.st_out.h, nominal = cfg_turb.st_out.h)), gas_iso(p(nominal = turbine.pstart_out), T(nominal = turbine.Tstart_out), h(start = cfg_turb.st_out.h))) annotation(
  ...
  Modelica.Mechanics.Rotational.Sources.ConstantSpeed const_speed_turb(w_fixed = cfg_turb.N, useSupport = false) annotation(
  ...
  Steps.TPComponents.TurbineFixedPController FPC_turb(redeclare package Medium = medium_main, fileName = Modelica.Utilities.Files.loadResource("modelica://Steps/Resources/Data/turbine_map.txt"), tablePhic = fill(0.0, 14, 12), Table = ThermoPower.Choices.TurboMachinery.TableTypes.file, Ndesign = cfg_turb.N, Tdes_in = cfg_turb.st_in.T, use_in_p2 = true, in_T1(start = cfg_turb.st_in.T, nominal = cfg_turb.st_in.T), in_p1(start = cfg_turb.st_in.p, nominal = cfg_turb.st_in.p), in_w1(start = cfg_turb.st_in.mdot, nominal = cfg_turb.st_in.mdot)) "Fixed pressure controller for main compressor" annotation(
    Placement(visible = true, transformation(origin = {-90, -41}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Mechanics.Rotational.Sources.Speed speed_turb(exact = false, w_ref(start = cfg_turb.N, nominal = cfg_turb.N), w(start = cfg_turb.N, nominal = cfg_turb.N)) annotation(
    Placement(visible = true, transformation(origin = {-73, -41}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  ThermoPower.Water.SourceMassFlow source_cooler(redeclare package Medium = medium_cooler, w0 = cfg_cooler.cfg_cold.st_in.mdot, p0 = cfg_cooler.cfg_cold.st_in.p, h = cfg_cooler.cfg_cold.st_in.h, T = cfg_cooler.cfg_cold.st_in.T) annotation(
    Placement(visible = true, transformation(origin = {56, -38}, extent = {{-4, -4}, {4, 4}}, rotation = 270)));
  ThermoPower.Water.SinkPressure sink_cooler(redeclare package Medium = medium_cooler, p0 = cfg_cooler.cfg_cold.st_out.p, T = cfg_cooler.cfg_cold.st_out.T, h = cfg_cooler.cfg_cold.st_out.h, use_T = false) annotation(
    Placement(visible = true, transformation(origin = {57, -97}, extent = {{-4, -4}, {4, 4}}, rotation = 270)));
  Steps.TPComponents.HE cooler(redeclare package FluidMedium = medium_cooler, redeclare package FlueGasMedium = medium_main, redeclare replaceable model HeatTransfer_F = Steps.TPComponents.NgoHeatTransferFV, redeclare replaceable model HeatTransfer_G = Steps.TPComponents.NgoHeatTransferFV, gasFlow(heatTransfer(Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 1500, gamma_max = 5000), Tstartin = cfg_cooler.cfg_gas.st_in.T, Tstartout = cfg_cooler.cfg_gas.st_out.T, Nt = cfg_cooler.cfg_gas.N_ch, Dhyd = cfg_cooler.cfg_gas.geo_area.d_hyd), fluidFlow(heatTransfer(Cf_C1 = Cf_C1, Cf_C2 = Cf_C2, Cf_C3 = Cf_C3, gamma_min = 2000, gamma_max = 18000), fixedMassFlowSimplified = true, hstartin = cfg_cooler.cfg_fluid.st_in.h, hstartout = cfg_cooler.cfg_fluid.st_out.h), redeclare model HeatExchangerTopology = ThermoPower.Thermal.HeatExchangerTopologies.CounterCurrentFlow, cfg = cfg_cooler, N_G = N_seg_cooler, N_F = N_seg_cooler, SSInit = true, gasQuasiStatic = false, FluidPhaseStart = ThermoPower.Choices.FluidPhase.FluidPhases.Liquid, metalTube(WallRes = false)) annotation(
  ...
  ThermoPower.Gas.Compressor compressor(redeclare package Medium = medium_main, pstart_in = cfg_comp.st_in.p, pstart_out = cfg_comp.st_out.p, Tstart_in = cfg_comp.st_in.T, Tstart_out = cfg_comp.st_out.T, tablePhic = tablePhic_comp_mc, tableEta = tableEta_comp_mc, tablePR = tablePR_comp_mc, Table = ThermoPower.Choices.TurboMachinery.TableTypes.matrix, Ndesign = cfg_comp.N, Tdes_in = cfg_comp.st_in.T, gas_in(p(start = cfg_comp.st_in.p, nominal = cfg_comp.st_in.p), T(start = cfg_comp.st_in.T, nominal = cfg_comp.st_in.T)), gas_iso(p(nominal = cfg_comp.st_in.p), T(nominal = cfg_comp.st_in.T), h(start = cfg_comp.st_out.h))) annotation(
    Placement(visible = true, transformation(origin = {162, -4}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
  Modelica.Mechanics.Rotational.Sources.ConstantSpeed const_speed_mc(w_fixed = cfg_comp.N, useSupport = false) annotation(
  ...
  Steps.TPComponents.CompressorFixedPController FPC_mc(redeclare package Medium = medium_main, tablePhic = tablePhic_comp_mc, tablePR = tablePR_comp_mc, Table = ThermoPower.Choices.TurboMachinery.TableTypes.matrix, Ndesign = cfg_comp.N, Tdes_in = cfg_comp.st_in.T, use_in_p2 = true, in_T1(start = cfg_comp.st_in.T, nominal = cfg_comp.st_in.T), in_p1(start = cfg_comp.st_in.p, nominal = cfg_comp.st_in.p), in_w1(start = cfg_comp.st_in.mdot, nominal = cfg_comp.st_in.mdot)) "Fixed pressure controller for main compressor" annotation(
    Placement(visible = true, transformation(origin = {129, -3}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Mechanics.Rotational.Sources.Speed speed_mc(exact = false, w_ref(start = cfg_comp.N, nominal = cfg_comp.N), w(start = cfg_comp.N, nominal = cfg_comp.N)) annotation(
    Placement(visible = true, transformation(origin = {145, -3}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  ThermoPower.Gas.Compressor recompressor(redeclare package Medium = medium_main, pstart_in = cfg_recomp.st_in.p, pstart_out = cfg_recomp.st_out.p, Tstart_in = cfg_recomp.st_in.T, Tstart_out = cfg_recomp.st_out.T, tablePhic = tablePhic_comp_rc, tableEta = tableEta_comp_rc, tablePR = tablePR_comp_rc, Table = ThermoPower.Choices.TurboMachinery.TableTypes.matrix, Ndesign = cfg_recomp.N, Tdes_in = cfg_recomp.st_in.T, gas_in(p(start = cfg_recomp.st_in.p, nominal = cfg_recomp.st_in.p), T(start = cfg_recomp.st_in.T, nominal = cfg_recomp.st_in.T)), gas_iso(p(nominal = cfg_recomp.st_in.p), T(nominal = cfg_recomp.st_in.T), h(start = cfg_recomp.st_out.h))) annotation(
    Placement(visible = true, transformation(origin = {97, -1}, extent = {{-9, -9}, {9, 9}}, rotation = 0)));
  Modelica.Mechanics.Rotational.Sources.ConstantSpeed const_speed_rc(w_fixed = cfg_recomp.N, useSupport = false) annotation(
  ...
  Steps.TPComponents.CompressorFixedPController FPC_rc(redeclare package Medium = medium_main, tablePhic = tablePhic_comp_rc, tablePR = tablePR_comp_rc, Table = ThermoPower.Choices.TurboMachinery.TableTypes.matrix, Ndesign = cfg_recomp.N, Tdes_in = cfg_recomp.st_in.T, use_in_p2 = true, in_T1(start = cfg_recomp.st_in.T, nominal = cfg_recomp.st_in.T), in_p1(start = cfg_recomp.st_in.p, nominal = cfg_recomp.st_in.p), in_w1(start = cfg_recomp.st_in.mdot, nominal = cfg_recomp.st_in.mdot)) "Fixed pressure controller for recompressor" annotation(
    Placement(visible = true, transformation(origin = {65, -3}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Mechanics.Rotational.Sources.Speed speed_rc(exact = false, w_ref(start = cfg_recomp.N, nominal = cfg_recomp.N), w(start = cfg_recomp.N, nominal = cfg_recomp.N)) annotation(
  ...
  constant Real time_scaling_factor = 1;
  // 1 hour = 1 hour
  // boundary conditions
  // ramp input to simulate the ramp change in ASPEN+ simulation
  // ramp change for case I in Guan's report
  Steps.TPComponents.RampSeq_W ramp_mdot(time_start = 3 * SEC_PER_HOUR, interval = 1 * SEC_PER_HOUR, duration = 0.15 * SEC_PER_HOUR, offset = st_source.mdot) annotation(
    Placement(visible = true, transformation(origin = {-146, 91}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Blocks.Sources.Constant p_const_TIP(k(start = cfg_turb.st_in.p, nominal = cfg_turb.st_in.p)) annotation(
    Placement(visible = true, transformation(origin = {-146, 63}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Blocks.Sources.Constant p_const_CIP(k(start = cfg_comp.st_in.p, nominal = cfg_comp.st_in.p)) annotation(
...
// Case I - variable speed - let FPC determine the shaft speed
  connect(r_turb_in.T, FPC_turb.in_T1) annotation(
    Line(points = {{-96, -28}, {-100, -28}, {-100, -41}, {-95, -41}}, color = {159, 159, 223}));
  connect(r_turb_in.w, FPC_turb.in_w1) annotation(
    Line(points = {{-100, -28}, {-100, -38}, {-95, -38}}, color = {159, 159, 223}));
  connect(p_const_TIP.y, FPC_turb.in_p1) annotation(
    Line(points = {{-140.5, 63}, {-136, 63}, {-136, -44}, {-94, -44}}, color = {0, 0, 127}));
// set the target inlet pressure
  connect(p_const_CIP.y, FPC_turb.in_p2) annotation(
    Line(points = {{-140.5, 79}, {-132, 79}, {-132, -32}, {-90, -32}, {-90, -36}}, color = {0, 0, 127}));
// set the target outlet pressure
  connect(FPC_turb.omega, speed_turb.w_ref) annotation(
    Line(points = {{-85, -41}, {-79, -41}}, color = {0, 0, 127}));
  connect(speed_turb.flange, turbine.shaft_a) annotation(
...
/*
  ...
  connect(splitter.outlet1, r_cooler_hin.inlet) annotation(
    Line(points = {{35, 18}, {35, -5.5}, {36, -5.5}, {36, -67}}, color = {170, 170, 255}, thickness = 0.5));
  connect(splitter.outlet2, r_recomp_in.inlet) annotation(
    Line(points = {{39, 18}, {45, 18}, {45, 19}, {44, 19}}, color = {170, 170, 255}, thickness = 0.5));
...
// for Case I - variable speed, fixed inlet and outlet pressure
  connect(p_const_CIP.y, FPC_rc.in_p1) annotation(
    Line(points = {{-140.5, 79}, {44, 79}, {44, -6}, {60, -6}}, color = {0, 0, 127}));
  connect(r_recomp_in.T, FPC_rc.in_T1) annotation(
    Line(points = {{50, 19}, {47.5, 19}, {47.5, -3}, {60, -3}}, color = {159, 159, 223}));
  connect(r_recomp_in.w, FPC_rc.in_w1) annotation(
    Line(points = {{50, 19}, {50.5, 19}, {50.5, 0}, {60, 0}}, color = {159, 159, 223}));
  connect(p_const_TIP.y, FPC_rc.in_p2) annotation(
    Line(points = {{-140, 64}, {-136, 64}, {-136, 72}, {64, 72}, {64, 2}, {66, 2}}, color = {0, 0, 127}));
// set the target outlet pressure
/*
      ...
      */
  connect(FPC_rc.omega, speed_rc.w_ref) annotation(
    Line(points = {{70, -3}, {68, -3}, {68, -2}, {75, -2}, {75, -3}}, color = {0, 0, 127}));
  connect(speed_rc.flange, recompressor.shaft_a) annotation(
...
// Case I - ramp mdot, controller determined varialbe speed, fixed in_p1 of controller
  connect(p_const_CIP.y, FPC_mc.in_p1) annotation(
    Line(points = {{-140.5, 79}, {114, 79}, {114, -6}, {124, -6}}, color = {0, 0, 127}));
  connect(r_comp_in.T, FPC_mc.in_T1) annotation(
    Line(points = {{120, 19}, {117, 19}, {117, -3}, {124, -3}}, color = {159, 159, 223}));
  connect(r_comp_in.w, FPC_mc.in_w1) annotation(
    Line(points = {{120, 19}, {119.5, 19}, {119.5, 0}, {124, 0}}, color = {159, 159, 223}));
  connect(p_const_TIP.y, FPC_mc.in_p2) annotation(
    Line(points = {{-140, 64}, {-135, 64}, {-135, 72}, {130, 72}, {130, 2}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
// set the target outlet pressure
/*
    ...
    */
  connect(FPC_mc.omega, speed_mc.w_ref) annotation(
    Line(points = {{134, -3}, {134, -2}, {140, -2}, {140, -3}, {139, -3}}, color = {0, 0, 127}));
  connect(speed_mc.flange, compressor.shaft_a) annotation(
...
// ramp input
// for case I
  connect(ramp_mdot.y, source.in_w0) annotation(
    Line(points = {{-140.5, 91}, {-128, 91}, {-128, -0.75}, {-110, -0.75}, {-110, -4.5}}, color = {0, 0, 127}));
// set target outlet pressure
  connect(p_const_TIP.y, sink.in_p0) annotation(
    Line(points = {{-140, 64}, {-136.25, 64}, {-136.25, 72}, {-102, 72}, {-102, 36}}, color = {0, 0, 127}));
/*
  // for case II 
