...
model TestTP_PCHE_GUI "GUI based model. To avoid the auto-formatting of OMC, which will reformat my code if use GUI to design the model"
  ...
  parameter Model.PBConfig_PCHE cfg(p_pump_in = p_hot_in, p_pump_out = p_cold_in, mdot_main = mdot_hot_in, mdot_pump = mdot_cold_in, T_LTR_cold_in = T_cold_in, T_LTR_cold_out = T_cold_out, T_HTR_hot_out = T_hot_in, T_LTR_hot_out = T_hot_out, r_LTR = r_ch, L_LTR = L_fp, N_ch_LTR = N_ch, N_seg = N_seg, pitch = L_pitch, phi = a_phi, rho_wall = rho_wall, cp_wall = cp_wall);
  // T_LTR_hot_in = T_HTR_hot_out,
  // set the values of parameters accordingly
  parameter HEBoundaryCondition bc_HE = cfg.bc_LTR;
  //Components
  inner ThermoPower.System system(allowFlowReversal = false, initOpt = ThermoPower.Choices.Init.Options.noInit) annotation(
    Placement(transformation(extent = {{80, 80}, {100, 100}})));
  ThermoPower.Gas.SourceMassFlow source_cold(redeclare package Medium = medium_cold, T = bc_HE.st_cold_in.T, p0 = bc_HE.st_cold_in.p, w0 = bc_HE.st_cold_in.mdot, gas(p(nominal = bc_HE.st_cold_in.p), T(nominal = bc_HE.st_cold_in.T))) annotation(
    Placement(visible = true, transformation(origin = {0, 80}, extent = {{-10, -10}, {10, 10}}, rotation = 270)));
  //use_in_T = false,
  ThermoPower.Gas.SinkPressure sink_cold(redeclare package Medium = medium_cold, p0 = bc_HE.st_cold_out.p, T = bc_HE.st_cold_out.T) annotation(
    Placement(transformation(origin = {0, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 270)));
  ThermoPower.Gas.SinkPressure sink_hot(redeclare package Medium = medium_hot, T = bc_HE.st_hot_out.T, p0 = bc_HE.st_hot_out.p) annotation(
    Placement(visible = true, transformation(extent = {{70, -10}, {90, 10}}, rotation = 0)));
  ThermoPower.Gas.SourceMassFlow source_hot(redeclare package Medium = medium_hot, T = bc_HE.st_hot_in.T, p0 = bc_HE.st_hot_in.p, w0 = bc_HE.st_hot_in.mdot, gas(p(nominal = bc_HE.st_hot_in.p), T(nominal = bc_HE.st_hot_in.T))) annotation(
  ...
  TPComponents.PCHE HE(redeclare package FluidMedium = medium_cold, redeclare package FlueGasMedium = medium_hot, redeclare replaceable model HeatTransfer_F = TPComponents.MarchionniPCHEHeatTransferFV, redeclare replaceable model HeatTransfer_G = TPComponents.MarchionniPCHEHeatTransferFV, gasFlow(heatTransfer(pitch = cfg.pitch, phi = cfg.phi, kc_dp = kc_dp, kc_cf = kc_cf_hot, use_rho_bar = use_rho_bar, rho_bar = rho_bar_hot)), fluidFlow(heatTransfer(pitch = cfg.pitch, phi = cfg.phi, kc_dp = kc_dp, kc_cf = kc_cf_cold, use_rho_bar = use_rho_bar, rho_bar = rho_bar_cold)), redeclare model HeatExchangerTopology = ThermoPower.Thermal.HeatExchangerTopologies.CounterCurrentFlow, bc = bc_HE, geo_hot = cfg.cfg_LTR_hot.geo, geo_cold = cfg.cfg_LTR_cold.geo, geo_tube = cfg.cfg_LTR_tube.geo, thermo_hot = cfg.cfg_LTR_hot.thermo, thermo_cold = cfg.cfg_LTR_cold.thermo, thermo_tube = cfg.cfg_LTR_tube.thermo, L = L_fp, SSInit = true, gasQuasiStatic = true, fluidQuasiStatic = true) annotation(
  ...
  Steps.TPComponents.GasStateReader sr_water_out annotation(
    Placement(visible = true, transformation(origin = {0, -48}, extent = {{-10, -10}, {10, 10}}, rotation = 270)));
equation
/*
  // HE alone
...
*/
  ...
  connect(source_hot.flange, sr_gas_in.inlet) annotation(
    Line);
  connect(sr_water_out.inlet, HE.waterOut) annotation(
  ...
  connect(sr_water_in.outlet, HE.waterIn) annotation(
    Line(points = {{0, 42}, {0, 42}, {0, 20}, {0, 20}}, color = {0, 0, 255}, thickness = 0.5));
  connect(sr_water_in.inlet, source_cold.flange) annotation(
    Line(points = {{0, 54}, {0, 54}, {0, 70}, {0, 70}}, color = {0, 0, 255}, thickness = 0.5));
  annotation(
    Diagram(graphics),
