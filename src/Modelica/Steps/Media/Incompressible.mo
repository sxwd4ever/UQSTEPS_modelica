...
which is only exactly true for a fluid with constant density d=d0.
    ...
    redeclare replaceable function extends density_derX "Returns the partial derivative of density with respect to mass fractions at constant pressure and temperature"
      algorithm
        dddX := fill(0, nX);
      annotation(
        Inline = true,
    ...
    /*
                    redeclare replaceable function extends density_derT "Density derivative by Temperature"
                      algorithm
    ...
    redeclare replaceable function extends density_derT_p "Returns the partial derivative of density with respect to mass fractions at constant pressure and temperature"
        /*
                              input ThermoPower.Gas.Flow1DFV$HE$gasFlow.Medium.ThermodynamicState state "Thermodynamic state record";
                            output Real ddTp(unit = "kg/(m3.K)") "Density derivative w.r.t. temperature";
                            */

      algorithm
        ddTp := Poly.derivativeValue(poly_rho, state.T);
      annotation(
        Inline = true,
        smoothOrder = 2);
    end density_derT_p;
