...
class ParamFitting(object):
    ...
    def __init__(self, exp, cfg_ref, cfg_offset_base, mapping, sim_ops, data_ref) -> None:
        super().__init__() 
        self.exp = exp
    ...
    def execute(self, valid = False):
        ...
        if valid and self.cfg_offset_base[name_test_set]:
    ...
    def param_random_local_search(self, func_para,pt,max_steps,num_samples,steplength):
        ...
        for i in range(max_steps):
            ...
            for j in range(num_samples):            
                ...
                x = rnd.uniform(-steplength_temp, steplength_temp) 
                y = rnd.uniform(-steplength_temp, steplength_temp) 
    ...
    def evaluate(self, x) -> Tuple[Tuple[float,float], TestDataSet]:

        (d_kc_h, d_kc_c) = x
        
        ds_exp = self.gen_experiment_ds(1 + d_kc_h, 1 + d_kc_c)

        self.exp.simulate(
            sim_ops=self.sim_ops,
            solution_dict=self.exp.gen_sol_dict(self.cfg_ref), 
            ds_test=ds_exp,
            append_save=False)  
    ...
    def gen_experiment_ds(self, kc_cf_h, kc_cf_c) -> TestDataSet:    

            # use result C1 for validation
            cfg_offset_cur = \
                {
                    "keys" : deepcopy(self.cfg_offset_base["keys"]),
                    "test": deepcopy(self.cfg_offset_base[self.stage])
                }

            # add kc_cf_hot, kc_cf_cold
            cfg_offset_cur['keys'].extend(['kc_cf_hot','kc_cf_cold'])            
            cfg_offset_cur["test"].extend([kc_cf_h, kc_cf_c])
            ...
            ds_exp = TestDataSet.gen_test_dataset(self.cfg_ref, cfg_offset, ds_name=exp_name)
    ...
    def cal_func_err(self, values):
        ...
        data_ref = self.gen_reference_data()[self.stage]
...
def main(work_root = []):
    ...
    if exp_type == ExpType.LOAD_PRE_EXP or exp_type == ExpType.VS_CFD:
        ...
        # High T for train and Low T for test
        # cfg_offset_base = {
        #         # values in Table 3 in Meshram [2016]
        #         # Meshram's DP result. 
        #         "keys" : ["T_cold_in", "T_cold_out", "T_hot_in", "T_hot_out", "u_cold_in", "u_hot_in", "a_phi","L_fp"],
        #         "train": [500, 615.48, 730, 601.83, 1.518, 6.118, 0, 0.2],
        #         "test": [400, 498.45, 630, 494.37, 0.842, 4.702, 0, 0.2]
        # }  

        # Low T for train and High T for test
        ...
        for name, case in case_dict.items():  
            fitting  = ParamFitting(exp, cfg_ref, case["cfg_offset"], mapping, sim_ops)
            fitting.execute()        
